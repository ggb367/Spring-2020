...
#get list of files
file_names = []
for path, subdirs, files in os.walk("Data/balanced"):
    for name in files:
...
x = [110, 130, 130, 150, 170, 170, 190, 210, 210, 230, 250, 50, 50, 70 , 90, 90, 0]
...
for avg in volt_average:
    strain.append((np.divide(np.multiply(-4,avg),(GF*(1+np.multiply(2, avg)))))*200*1000)
for mass in x:
    force = mass*9.81/1000
    strain_theory.append(force*L*thic/(2*young_mod*I)*1000)

plt.scatter(x, np.abs(strain-strain[16]), label='Measured')
plt.errorbar(x, np.abs(strain-(strain[16])), np.std(strain), fmt='none')
plt.plot(x, strain_theory, label='Theory')
plt.legend(loc=4)
plt.xlabel("Mass (Grams)")
plt.ylabel("Strain (Millistrain)")
plt.title("Measured Strain From Balanced Bridge vs Mass ")
plt.grid(True)
